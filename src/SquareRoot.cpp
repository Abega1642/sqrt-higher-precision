...
SquareRoot::SquareRoot(const unsigned long natural_number)
	: nat(natural_number),
	  root_value(static_cast<unsigned long>(std::ceil(std::sqrt(static_cast<double>(nat))))),
...
unsigned long SquareRoot::get_nat() const {
	return nat;
}
...
unsigned long SquareRoot::get_root_value() const {
	return root_value;
}
