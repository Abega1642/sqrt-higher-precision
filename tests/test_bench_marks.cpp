...
void show_result(
	...
	const unsigned long nat,
	const unsigned long range,
	const unsigned long precision_digits,
	const double time
) {
...
TEST(SquareRootBenchmark, TimeExecutionOfKthValueComputation) {
	...
	show_result("Fraction form", subject.get_nat(), k, precision_digits, elapsed_seconds.count());
...
TEST(SquareRootBenchmark, TimeExecutionOfKthValueComputationWithSixMillionPreision) {
	constexpr unsigned long target = 2UL;
	constexpr unsigned long k = 22UL;
	constexpr std::size_t precision_digits = 6'000'002;
	...
	show_result("Decimal Form", subject.get_nat(), k, precision_digits, elapsed_seconds.count());
